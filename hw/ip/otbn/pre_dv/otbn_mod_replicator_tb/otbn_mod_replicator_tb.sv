...
module otbn_mod_replicator_tb
  ...
  initial begin : application_block
    ...
    stim_fd = $fopen("./otbn_mod_replicator_tb/otbn_mod_replicator.golden", "r");
    if (stim_fd == 0) begin
    ...
    while (!$feof(stim_fd)) begin
      ...
      #(APPL_DELAY);
      // Set inputs
      ret_code = $fscanf(stim_fd, "%d, 0x%h, 0x%h\n",
                         elen_size, mod, gold_response.mod_rep);
      if (ret_code == 0) begin
      ...
      $display("Testing ELEN = %d ", elen,
               "mod = 0x%h ", mod,
               "mod_rep = 0x%h ", gold_response.mod_rep);
