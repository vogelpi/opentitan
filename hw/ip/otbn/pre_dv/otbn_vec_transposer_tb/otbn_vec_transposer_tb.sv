...
module otbn_vec_transposer_tb
  ...
  initial begin : application_block
    ...
    stim_fd = $fopen("./otbn_vec_transposer_tb/otbn_vec_transposer.golden", "r");
    if (stim_fd == 0) begin
    ...
    while (!$feof(stim_fd)) begin
      ...
      #(APPL_DELAY);
      // Set inputs
      ret_code = $fscanf(stim_fd, "%d, 0x%h, 0x%h, %d, 0x%h\n",
                         elen_size, operand_a, operand_b, is_trn1, gold_response.result);
      if (ret_code == 0) begin
