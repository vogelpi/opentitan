...
#(
  ...
  prim_trivium #(
   .BiviumVariant         (1),
   .OutputWidth           (Width),
   .LockupProtection      (!SecAllowForcingMasks),
  ...
  // - If SecAllowForcingMasks == 0, the primitive is automatically reset to the default value
  //   defined by a secret netlist constant.
  // - If SecAllowForcingMasks == 1, the primitive keeps the all-zero value. This may be required
  //   to switch off the masking for evaluating the SCA resistance.
