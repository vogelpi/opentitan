...
#(
  ...
  if (AESGCMEnable) begin : gen_ghash
    logic [3:0][3:0][7:0] ghash_state_done [NumShares];
    ...
    aes_ghash #(
      ...
      .ghash_state_done_o  ( ghash_state_done       )
    );

    if (!SecMasking) begin : gen_ghash_state_out_unmasked
      assign ghash_state_out = ghash_state_done[0];
    end else begin : gen_ghash_state_out_masked
      // Only unmask the final GHASH output, i.e., the final authentication tag. Unmasking
      // intermediate output data may cause additional SCA leakage and thus should be avoided.
      // Forward PRD instead of a determinsitic value to avoid leaking the GHASH output when it
      // becomes valid.
      logic [3:0][3:0][7:0] ghash_state_done_muxed [NumShares];
      for (genvar s = 0; s < NumShares; s++) begin : gen_ghash_state_done_muxed
        assign ghash_state_done_muxed[s] =
            (ghash_out_valid == SP2V_HIGH) ? ghash_state_done[s] : prd_clearing_128[s];
      end
      ...
      prim_buf #(
        ...
        .in_i  ( {ghash_state_done_muxed[1], ghash_state_done_muxed[0]} ),
        .out_o ( {ghash_state_done_buf[1],   ghash_state_done_buf[0]}   )
      );

      // Unmask the cipher core output.
      assign ghash_state_out = ghash_state_done_buf[0] ^ ghash_state_done_buf[1];
    end
