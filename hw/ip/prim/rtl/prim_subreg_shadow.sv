...
#(
  ...
  // - Restored to the committed value in Phase 1 in case of update errors.
  // - Once storage error occurs, do not allow any further update until reset.
  if (InvertedSwAccess != SwAccess) begin : gen_two_prim_subreg_arb
    ...
    assign shadow_we = ~err_storage & (phase_clear |
                                          (!phase_q ? shadow_wr_en_phase0 :
                                                      shadow_wr_en_phase1 | err_update));
    assign shadow_wd = !phase_q    ? shadow_wr_data_phase0 :
                       !err_update ? shadow_wr_data_phase1 : ~committed_q;
  ...
  end else begin : gen_prim_subreg
    // The standard shadow register:
    // - Holds the 1's complement value.
    // - Unconditionally written in Phase 0.
    // - Restored to the committed value in Phase 1 in case of update errors.
    // - Once storage error occurs, do not allow any further update until reset.
    ...
    logic          shadow_de;
    logic [DW-1:0] shadow_d;

    assign shadow_we = ~err_storage & (phase_clear | (!phase_q ? we : err_update));
    assign shadow_de = ~err_storage & (phase_clear | (!phase_q ? de : err_update));
    assign shadow_wd = phase_clear || (phase_q && err_update) ? ~committed_q : ~wr_data;
    assign shadow_d  = phase_clear || (phase_q && err_update) ? ~committed_q : ~d;
  ...
  // - Storage errors are only signaled in Phase 0. In Phase 1, the two registers are expected to
  //   be out of sync.
